...
bool IsInner(Point& p, vector<Point>& convexHull)
{
    ...
    if (Point::CCW(convexHull[0], convexHull[1], p) < 0)
        return false;
    if (Point::CCW(convexHull[0], convexHull[n - 1], p) > 0)
        return false;
    ...
    while (start + 1 < end)
    {
        ...
        if (Point::CCW(convexHull[0], convexHull[mid], p) >= 0)
            start = mid;
        else
            end = mid;
    }
    return Point::CCW(convexHull[start], convexHull[end], p) >= 0;
}
